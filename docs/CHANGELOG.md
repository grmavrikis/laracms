...
## 64. §63's caret sat wrong on a short row, the card wanted colour, and Preview's own tooltip was unreachable

Three small reports against the card §63 had just shipped.

**The caret came back off.** Anchored to the card's own bottom corner, it
assumed the card's vertical middle lined up with two lines of wrapped
description - true for the row it was built against, wrong for any row whose
own content made it a different height. Removed rather than re-anchored: the
card's **border** now carries a light accent tint (`border-accent/25`,
`shadow-accent/10` alongside the existing `shadow-lg`) instead, which ties it
back to its row by colour rather than by a shape measured from one corner -
a treatment a row of any height renders identically.

**A little colour, asked for by name.** `IconButton` gains an `accent` tone
- `text-accent-text`, tinted at rest rather than only on hover the way
`danger` is, because the point is to stand out from its neighbours rather
than to warn - and Edit, the row's one primary action among four, is the
only button that takes it. The reorder arrows, the divider and Preview stay
exactly as neutral as before: one touch of colour, not a colourful toolbar.

**Preview stopped being `disabled`.** It looked and behaved as though nothing
was there to press, which was the report - "let us see it normally." Reading
`BulkButton`'s own comment on this codebase's established rule turned up why
that was worse than it looked: a disabled control does not reliably fire the
hover that reveals its own `title`, so the "not wired yet" this button has
always carried was, in practice, close to unreachable for a sighted mouse
user. It is a plain enabled button now - same neutral tone as the reorder
arrows, no `onClick` because it still has nowhere to go (#136) - and the
label is exactly as before, so `aria-label`/`title` still say the one true
thing about it.

### Checked

Five tests changed. `overlayFor(11).querySelector('.rotate-45')` now asserts
the caret is *absent*; a new test checks the card's `className` for
`border-accent/`; a new test checks Edit's `className` for `text-accent-text`.
The two Preview tests were flipped from `toBeDisabled()` to
`not.toBeDisabled()` plus a new one confirming a click does nothing and does
not also toggle the row's own selection - the guard `clickedControl` already
gives every other button in the row. All five confirmed to fail against the
pre-change component first.

Live: `getComputedStyle` on the card read `border-color:
oklab(… / 0.25)` and a two-layer `box-shadow` both resolving to
`--ui-accent`, and Edit's own `color` resolved to exactly `--ui-accent-text`
(`#047857` in light, `#34d399` in dark, both re-pointed correctly with no
theme-specific code written for this) - checked without needing hover to
fire, since none of the three is conditioned on the reveal state, only on the
group's own visibility classes as before. A real keyboard `Tab` into the row
revealed the card with the tinted border and the coloured pencil, no caret
underneath, at a screenshot scaled 3× to see it - and Preview's icon sat at
full, undimmed opacity beside it, no longer looking like a control that does
not respond. Checked in dark theme and confirmed the accessibility tree still
names Preview `Προεπισκόπηση — δεν έχει συνδεθεί ακόμη` while reporting it as
