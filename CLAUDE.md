...
- **#129, a colour bleed and a singleton's active click, is DONE**
  ...
  still-visible box, so the two rows' colours transitioned at once (§51).
  Skipping that fade was real but not the whole story: checked again, the
  owner's report came back unchanged, because the `background-color`
  transition itself (added in §49) was interpolating between the accent green
  and the sidebar grey through a muddy in-between — **§52 removed that
  transition entirely; the colour now snaps.** Separately, a singleton's own
  row is active at `entryEdit` but its link points at the listing, which
  redirects straight back — clicking it was a real, visible round trip. Fixed
  with one guard: an already-active row's click does nothing. All of it
  verified live, including sampling the flyout every 10ms through a hover
  swap both before and after §52.
